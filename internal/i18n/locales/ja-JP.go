...
var MessagesJaJP = map[string]string{
	...
	"config.enable_key_fallback":             "キーフォールバックを有効化",
	"config.enable_key_fallback_desc":        "有効にすると、上流が HTTP エラー（ステータス ≥ 400）またはネットワークエラーを返した場合、フェイルオーバーステータスコードの設定に関わらず、グループ内の次のキーに自動的に切り替えます。最大リトライ回数は「最大リトライ数」の設定に従います。",
	"config.rate_limit_rpm":                  "レートリミット（1分あたりの最大リクエスト数）",
	"config.rate_limit_rpm_desc":             "このグループの上流プロバイダーに送信する1分あたりの最大リクエスト数。0で無制限。制限を超えると429エラーを返します。",
