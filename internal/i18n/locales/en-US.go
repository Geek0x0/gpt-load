...
var MessagesEnUS = map[string]string{
	...
	"config.enable_key_fallback":             "Enable Key Fallback",
	"config.enable_key_fallback_desc":        "When enabled, any HTTP error (status ≥ 400) or network error will automatically switch to the next key in the group, regardless of the Failover Status Codes setting. The maximum number of retries is still controlled by Max Retries.",
	"config.rate_limit_rpm":                  "Rate Limit (Requests Per Minute)",
	"config.rate_limit_rpm_desc":             "Maximum number of requests per minute sent to this group's upstream provider. 0 means unlimited. Returns 429 when the limit is exceeded.",
