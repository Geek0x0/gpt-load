...
var MessagesZhCN = map[string]string{
	...
	"config.enable_key_fallback":             "启用 Key 故障转移",
	"config.enable_key_fallback_desc":        "启用后，当上游返回任意 HTTP 错误（状态码 ≥ 400）或发生网络错误时，均会自动切换至分组内的下一个 Key 重试，不再受「故障转移状态码」限制。最大重试次数仍受「最大重试次数」配置控制。",
	"config.rate_limit_rpm":                  "上游限速（每分钟最大请求数）",
	"config.rate_limit_rpm_desc":             "限制每分钟向该分组上游发出的最大请求数，0 表示不限速。超出限制时返回 429 错误。",
